 ...
 *
 * When using object-fit: cover, the video is scaled by `s = max(Cw / Vw, Ch / Vh)` so that one dimension exactly fills
 * the container while the other overflows. The overflowing dimension is then centered and cropped.
 *
 ...
 *
 * The croppedFraction represents the relative amount (fraction) of the video that is cropped along the overflowing
 * dimension. We choose "cover" only when the croppedFraction is below 10%.
 *
 ...
 */
export const determineFitMode = (Cw: number, Ch: number, Vw: number, Vh: number): FitMode => {
