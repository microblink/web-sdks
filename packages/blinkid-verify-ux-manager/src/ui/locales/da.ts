/** Copyright (c) 2026 Microblink Ltd. All rights reserved. */

/** Localization strings for da. Generated by core-i18n. */
export default {
  document_filtered_modal: {
    details: "Prøv at scanne et andet dokument.",
    title: "Dokument ikke accepteret",
  },
  document_not_recognized_modal: {
    details: "Scan forsiden af et understøttet dokument.",
    title: "Dokument ikke genkendt",
  },
  feedback_messages: {
    blur_detected: "Hold dokumentet og telefonen stille",
    camera_angle_too_steep: "Hold dokumentet parallelt med telefonen",
    document_scanned_aria: "Det lykkedes! Dokumentet er scannet.",
    document_too_close_to_edge: "Flyt længere væk",
    face_photo_not_fully_visible: "Sørg for, at ansigtsbilledet er helt synligt",
    flip_document: "Vend dokumentet",
    flip_to_back_side: "Vend på bagsiden",
    front_side_scanned_aria: "Det lykkedes! Forsiden er scannet.",
    glare_detected: "Vip eller flyt dokumentet for at fjerne genskin",
    keep_document_parallel: "Hold dokumentet parallelt med skærmen",
    keep_document_still: "Undgå at bevæge tingene",
    move_closer: "Flyt tættere på",
    move_farther: "Flyt længere væk",
    move_left: "Flyt til den venstre side",
    move_right: "Flyt til den højre side",
    move_top: "Flyt til den øverste side",
    occluded: "Hold hele dokumentet synligt",
    scan_data_page: "Scan dokumentets dataside",
    scan_last_page_barcode: "Scan stregkoden fra den sidste side",
    scan_left_page: "Scan den venstre side",
    scan_right_page: "Scan den højre side",
    scan_the_back_side: "Scan bagsiden af et dokument",
    scan_the_barcode: "Scan stregkoden",
    scan_the_front_side: "Scan forsiden af et dokument",
    scan_top_page: "Scan den øverste side",
    screen_detected: "Flyt dokumentet til en ensfarvet baggrund",
    too_bright: "Flyt til en sted med mindre lys",
    too_dark: "Flyt til et lysere sted",
    wrong_left: "Flyt til den venstre side",
    wrong_right: "Flyt til den højre side",
    wrong_top: "Flyt til den øverste side",
  },
  help_button: {
    aria_label: "Hjælp",
    tooltip: "Brug for hjælp?",
  },
  help_modal: {
    aria: "Hjælp til scanning",
    back_btn: "Tilbage",
    blur: {
      details: "Prøv at holde telefonen og dokumentet stille, mens du scanner. Bevægelse kan enten sløre billedet eller gøre dataene på dokumentet ulæselige.",
      details_desktop: "Prøv at holde enheden og dokumentet stille, mens du scanner. Bevægelse kan enten sløre billedet eller gøre dataene på dokumentet ulæselige.",
      title: "Undgå bevægelse, mens du scanner",
    },
    camera_lens: {
      details: "Tjek, om der er snavs eller støv på din kameralinse. En snavset linse gør billedet sløret, hvilket gør dokumentets oplysninger ulæselige og forhindrer scanning af dataene.",
      title: "Rens din kameralinse",
    },
    done_btn: "Færdig",
    done_btn_aria: "Genoptag scanning",
    lighting: {
      details: "Undgå direkte og skarpt lys, fordi de giver genskin fra dokumentet og kan gøre dele af dokumentet ulæselige. Hvis du ikke kan læse dataene på dokumentet, vil de ikke være synlige på kameraet.",
      title: "Vær opmærksom på skarpt lys",
    },
    next_btn: "Næste",
    visibility: {
      details: "Sørg for, at du ikke dækker nogen dele af dokumentet med en finger, herunder stregerne i bunden. Vær også opmærksom på hologramreflektioner, der går henover dokumentets felter.",
      title: "Hold alle felterne synlige",
    },
  },
  onboarding_modal: {
    aria: "Instruktioner i scanning",
    btn: "Start scanning",
    details: "Sørg for, at dokumentet er godt oplyst. Alle dokumentets felter skal være synlige på kameraskærmen.",
    details_desktop: "Sørg for, at kameralinsen er ren, og dokumentet er belyst. Alle dokumentets felter skal være synlige på kameraskærmen.",
    title: "Hold alle oplysningerne synlige",
    title_desktop: "Gør dig klar til at scanne",
  },
  sdk_aria: "Skærm til dokumentscanning",
  timeout_modal: {
    cancel_btn: "Annuller scanning",
    details: "Sørg for, at dokumentet er godt belyst og fuldt ud synligt, samt at der ikke er noget genskin.",
    retry_btn: "Prøv igen",
    title: "Dokumentet kunne ikke læses",
  },
} as const;
