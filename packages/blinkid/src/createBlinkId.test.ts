...
const {
  ...
  mockAddOnUiStateChangedCallback,
  mockAddOnErrorCallback,
  mockAddOnFrameProcessCallback,
  mockAddOnProgressCallback,
  ...
  const mockAddOnUiStateChangedCallback = vi.fn();
  const mockAddOnErrorCallback = vi.fn();
  const mockAddOnFrameProcessCallback = vi.fn();
  const mockAddOnProgressCallback = vi.fn();
  ...
  const mockCreateBlinkIdUxManager = vi.fn().mockResolvedValue({
    ...
    addOnUiStateChangedCallback: mockAddOnUiStateChangedCallback,
    addOnErrorCallback: mockAddOnErrorCallback,
    addOnFrameProcessCallback: mockAddOnFrameProcessCallback,
    addOnProgressCallback: mockAddOnProgressCallback,
...
describe("createBlinkId", () => {
  ...
  test("returns a BlinkIdComponent with all required properties", async () => {
    ...
    expect(component).toHaveProperty("addOnUiStateChangedCallback");
    expect(component).toHaveProperty("addOnProgressCallback");
    ...
    expect(typeof component.addOnProgressCallback).toBe("function");
  ...
  test("passes uxManagerOptions to createBlinkIdUxManager", async () => {
    ...
    } satisfies NonNullable<BlinkIdComponentOptions["uxManagerOptions"]>;
  ...
  test("BlinkIdComponent callback helpers invoke UX manager methods", async () => {
    const component = await createBlinkId({ licenseKey: "test-key" });
    const resultCb = vi.fn();
    const errorCb = vi.fn();
    const frameProcessCb = vi.fn();
    const uiStateCb = vi.fn();
    ...
    component.addOnUiStateChangedCallback(uiStateCb);
    component.addOnFrameProcessCallback(frameProcessCb);
    component.addOnProgressCallback(progressCb);
    ...
    expect(mockAddOnUiStateChangedCallback).toHaveBeenCalledWith(uiStateCb);
    expect(mockAddOnFrameProcessCallback).toHaveBeenCalledWith(frameProcessCb);
    expect(mockAddOnProgressCallback).toHaveBeenCalledWith(progressCb);
