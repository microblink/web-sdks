...
/**
 * Returns the next chained UI state key after a state transition, or `undefined` if there is no automatic follow-on
 * state.
 *
 * BlinkCard chain:
 *
 ...
 */
export function getBlinkCardChainedUiStateKey(props: BlinkCardChainedUiStateProps): BlinkCardUiStateKey | undefined {
  return match<BlinkCardChainedUiStateProps, BlinkCardUiStateKey | undefined>(props)
