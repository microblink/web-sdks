...
[@microblink/blinkid-ux-manager](../../README.md) / [index](../README.md) / BlinkIdFrameProcessCallback
...
Process result for the current frame. This contains the input image analysis and result
...
Advances the scanning session to the next required step. Use this for custom flows that
...
Immediately triggers the scan-step timeout path for the active step. Use this when custom
...
Returns the raw `ArrayBuffer` for the frame that produced `frameResult`. The buffer is intended
