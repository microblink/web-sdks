...
describe("modal extraction mode content", () => {
  test("maps onboarding extraction modes to locale groups", () => {
    expect(
      Object.fromEntries(
        Object.entries(onboardingModalContentByExtractionMode).map(([extractionMode, content]) => [
          extractionMode,
          content.localeGroup,
        ]),
      ),
...
describe("modal image fallbacks", () => {
  test("keeps onboarding illustrations extraction-mode-specific", () => {
    const fullDocument = onboardingModalContentByExtractionMode["full-document"];
    const documentWithBarcode = onboardingModalContentByExtractionMode["document-with-barcode"];
    const barcodeOnly = onboardingModalContentByExtractionMode["barcode-only"];

    expect(documentWithBarcode.images.mobile).not.toBe(fullDocument.images.mobile);
    expect(documentWithBarcode.images.desktop).not.toBe(fullDocument.images.desktop);
  ...
  test("falls back to default help images for missing extraction-mode-specific illustrations", () => {
    const fullDocumentSteps = helpModalContentByExtractionMode["full-document"].scanSteps;
    const documentWithBarcodeSteps = helpModalContentByExtractionMode["document-with-barcode"].scanSteps;
  ...
  test("uses extraction-mode-specific help images when available", () => {
    const fullDocumentSteps = helpModalContentByExtractionMode["full-document"].scanSteps;
    const documentWithBarcodeSteps = helpModalContentByExtractionMode["document-with-barcode"].scanSteps;
    const barcodeOnlySteps = helpModalContentByExtractionMode["barcode-only"].scanSteps;

    expect(documentWithBarcodeSteps[0].image).not.toBe(fullDocumentSteps[0].image);
