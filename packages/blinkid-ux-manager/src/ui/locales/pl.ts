/**
 * Copyright (c) 2026 Microblink Ltd. All rights reserved.
 */

/**
 * Localization strings for pl.
 */
export default {
  document_filtered_modal: {
    details: "Spróbuj zeskanować inny dokument",
    title: "Dokument nie został zaakceptowany",
  },
  document_not_recognized_modal: {
    details: "Zeskanuj przednią stronę obsługiwanego dokumentu.",
    title: "Nie rozpoznano dokumentu",
  },
  error_modal: { cancel_btn: "Anuluj", retry_btn: "Spróbuj ponownie" },
  feedback_messages: {
    blur_detected: "Trzymaj dokument i telefon nieruchomo",
    camera_angle_too_steep: "Trzymaj dokument równolegle do telefonu",
    document_scanned_aria: "Udało się! Zeskanowano dokument",
    document_too_close_to_edge: "Przesuń dalej",
    face_photo_not_fully_visible:
      "Zdjęcie twarzy powinno być w całości widoczne",
    flip_document: "Odwróć dokument",
    flip_to_back_side: "Obróć na drugą stronę",
    front_side_scanned_aria: "Udało się! Zeskanowano przednią stronę",
    glare_detected: "Przechyl lub przesuń dokument, aby usunąć odbicie światła",
    keep_document_parallel: "Trzymaj dokument równolegle do ekranu",
    keep_still: "Nie ruszaj się",
    move_closer: "Przesuń bliżej",
    move_farther: "Przesuń dalej",
    move_left: "Przejdź do strony po lewej",
    move_right: "Przejdź do strony po prawej",
    move_top: "Przejdź do strony na górze",
    occluded: "Dokument powinien być w całości widoczny",
    scan_data_page: "Zeskanuj stronę dokumentu z danymi",
    scan_last_page_barcode: "Zeskanuj kod kreskowy z ostatniej strony",
    scan_left_page: "Zeskanuj lewą stronę",
    scan_right_page: "Zeskanuj prawą stronę",
    scan_the_back_side: "Zeskanuj tylną stronę dokumentu",
    scan_the_barcode: "Zeskanuj kod kreskowy",
    scan_the_barcode_side: "Zeskanuj stronę dokumentu z kodem kreskowym",
    scan_the_front_side: "Zeskanuj przednią stronę dokumentu",
    scan_the_mrz_side: "Zeskanuj stronę dokumentu ze strefą MRZ",
    scan_top_page: "Zeskanuj górną stronę",
    too_bright: "Przejdź w mniej oświetlone miejsce",
    too_dark: "Przejdź w jaśniejsze miejsce",
    wrong_left: "Przejdź do lewej strony",
    wrong_right: "Przejdź do prawej strony",
    wrong_top: "Przejdź do górnej strony",
  },
  help_button: { aria_label: "Pomoc", tooltip: "Potrzebujesz pomocy?" },
  help_modal: {
    aria: "Pomoc dotycząca skanowania",
    back_btn: "Wstecz",
    barcode_only: {
      blur: {
        details:
          "Podczas skanowania staraj się trzymać telefon i kod kreskowy nieruchomo. Poruszanie którymkolwiek z nich może rozmyć obraz i utrudnić odczytanie kodu kreskowego.",
        details_desktop:
          "Staraj się nie ruszać podczas skanowania. Ruch może rozmyć obraz i utrudnić odczytanie kodu kreskowego.",
        title: "Nie ruszaj się podczas skanowania",
        title_desktop: "Nie ruszaj się podczas skanowania",
      },
      camera_lens: {
        details_desktop:
          "Sprawdź obiektyw aparatu pod kątem smug lub kurzu. Brudny obiektyw powoduje rozmycie końcowego obrazu, przez co kod kreskowy staje się nieczytelny i uniemożliwia pomyślne zeskanowanie danych.",
        title_desktop: "Wyczyść obiektyw aparatu",
      },
      lighting: {
        details:
          "Unikaj bezpośredniego, ostrego światła, ponieważ może ono powodować odblaski na kodzie kreskowym i utrudniać jego skanowanie. Jeśli kod kreskowy nie jest dla Ciebie wyraźnie widoczny, aparat również może mieć problem z jego odczytaniem.",
        details_desktop:
          "Unikaj bezpośredniego, ostrego światła, ponieważ może ono powodować odblaski na kodzie kreskowym i utrudniać jego skanowanie. Jeśli kod kreskowy nie jest dla Ciebie wyraźnie widoczny, aparat również może mieć problem z jego odczytaniem.",
        title: "Uważaj na silne światło",
        title_desktop: "Uważaj na silne światło",
      },
      visibility: {
        details:
          "Upewnij się, że nie zasłaniasz palcem żadnej części kodu kreskowego. Uważaj też na odbicia światła na kodzie kreskowym, które mogą utrudnić jego odczytanie.",
        details_desktop:
          "Upewnij się, że nie zasłaniasz palcem żadnej części kodu kreskowego. Uważaj też na odbicia światła na kodzie kreskowym, które mogą utrudnić jego odczytanie.",
        title: "Upewnij się, że kod kreskowy jest widoczny",
        title_desktop: "Upewnij się, że kod kreskowy jest widoczny",
      },
    },
    document_with_barcode: {
      blur: {
        details:
          "Staraj się trzymać telefon i dokument nieruchomo podczas skanowania. Poruszanie którymkolwiek z nich może spowodować rozmazanie obrazu i uniemożliwić odczyt danych z dokumentu.",
        details_desktop:
          "Staraj się nie ruszać podczas skanowania. Ruch może rozmyć obraz i sprawić, że dane na dokumencie staną się nieczytelne.",
        title: "Nie ruszaj się podczas skanowania",
        title_desktop: "Nie ruszaj się podczas skanowania",
      },
      camera_lens: {
        details_desktop:
          "Sprawdź, czy obiektyw aparatu nie jest zabrudzony lub zakurzony. Zabrudzenia mogą powodować rozmycie obrazu, przez co szczegóły dokumentu stają się nieczytelne, a ich zeskanowanie niemożliwe.",
        title_desktop: "Wyczyść obiektyw aparatu",
      },
      lighting: {
        details:
          "Unikaj bezpośredniego silnego światła, ponieważ odbija się od dokumentu i niektóre jego części mogą stać się nieczytelne. Jeśli nie możesz odczytać danych z dokumentu, nie będą one widoczne również dla aparatu.",
        details_desktop:
          "Unikaj bezpośredniego silnego światła, ponieważ odbija się od dokumentu i niektóre jego części mogą stać się nieczytelne. Jeśli nie możesz odczytać danych z dokumentu, nie będą one widoczne również dla aparatu.",
        title: "Uważaj na silne światło",
        title_desktop: "Uważaj na silne światło",
      },
      visibility: {
        details:
          "Upewnij się, że nie zasłaniasz palcem żadnej części kodu kreskowego. Uważaj też na odbicia światła na kodzie kreskowym, które mogą utrudnić jego odczytanie.",
        details_desktop:
          "Upewnij się, że nie zasłaniasz palcem żadnej części kodu kreskowego. Uważaj też na odbicia światła na kodzie kreskowym, które mogą utrudnić jego odczytanie.",
        title: "Upewnij się, że kod kreskowy jest widoczny",
        title_desktop: "Upewnij się, że kod kreskowy jest widoczny",
      },
    },
    document_with_mrz: {
      blur: {
        details:
          "Staraj się trzymać telefon i dokument nieruchomo podczas skanowania. Poruszanie którymkolwiek z nich może spowodować rozmazanie obrazu i uniemożliwić odczyt danych z dokumentu.",
        details_desktop:
          "Staraj się nie ruszać podczas skanowania. Ruch może rozmyć obraz i sprawić, że dane na dokumencie staną się nieczytelne.",
        title: "Nie ruszaj się podczas skanowania",
        title_desktop: "Nie ruszaj się podczas skanowania",
      },
      camera_lens: {
        details_desktop:
          "Sprawdź, czy obiektyw aparatu nie jest zabrudzony lub zakurzony. Zabrudzenia mogą powodować rozmycie obrazu, przez co szczegóły dokumentu stają się nieczytelne, a ich zeskanowanie niemożliwe.",
        title_desktop: "Wyczyść obiektyw aparatu",
      },
      lighting: {
        details:
          "Unikaj bezpośredniego silnego światła, ponieważ odbija się od dokumentu i niektóre jego części mogą stać się nieczytelne. Jeśli nie możesz odczytać danych z dokumentu, nie będą one widoczne również dla aparatu.",
        details_desktop:
          "Unikaj bezpośredniego silnego światła, ponieważ odbija się od dokumentu i niektóre jego części mogą stać się nieczytelne. Jeśli nie możesz odczytać danych z dokumentu, nie będą one widoczne również dla aparatu.",
        title: "Uważaj na silne światło",
        title_desktop: "Uważaj na silne światło",
      },
      visibility: {
        details:
          "Upewnij się, że nie zasłaniasz palcem żadnej części strefy MRZ. Uważaj też na odbicia światła na strefie MRZ, które mogą utrudnić jej odczytanie.",
        details_desktop:
          "Upewnij się, że nie zasłaniasz palcem żadnej części strefy MRZ. Uważaj też na odbicia światła na strefie MRZ, które mogą utrudnić jej odczytanie.",
        title: "Upewnij się, że strefa MRZ jest widoczna",
        title_desktop: "Upewnij się, że strefa MRZ jest widoczna",
      },
    },
    done_btn: "Gotowe",
    done_btn_aria: "Wznów skanowanie",
    full_document: {
      blur: {
        details:
          "Staraj się trzymać telefon i dokument nieruchomo podczas skanowania. Poruszanie którymkolwiek z nich może spowodować rozmazanie obrazu i uniemożliwić odczyt danych z dokumentu.",
        details_desktop:
          "Staraj się nie ruszać podczas skanowania. Ruch może rozmyć obraz i sprawić, że dane na dokumencie staną się nieczytelne.",
        title: "Nie ruszaj się podczas skanowania",
        title_desktop: "Nie ruszaj się podczas skanowania",
      },
      camera_lens: {
        details_desktop:
          "Sprawdź, czy obiektyw aparatu nie jest zabrudzony lub zakurzony. Zabrudzenia mogą powodować rozmycie obrazu, przez co szczegóły dokumentu stają się nieczytelne, a ich zeskanowanie niemożliwe.",
        title_desktop: "Wyczyść obiektyw aparatu",
      },
      lighting: {
        details:
          "Unikaj bezpośredniego silnego światła, ponieważ odbija się od dokumentu i niektóre jego części mogą stać się nieczytelne. Jeśli nie możesz odczytać danych z dokumentu, nie będą one widoczne również dla aparatu.",
        details_desktop:
          "Unikaj bezpośredniego silnego światła, ponieważ odbija się od dokumentu i niektóre jego części mogą stać się nieczytelne. Jeśli nie możesz odczytać danych z dokumentu, nie będą one widoczne również dla aparatu.",
        title: "Uważaj na silne światło",
        title_desktop: "Uważaj na silne światło",
      },
      visibility: {
        details:
          "Upewnij się, że nie zasłaniasz dokumentu, w tym jego dolnych krawędzi, palcem. Zwróć też uwagę na refleksy z hologramu na polach dokumentu.",
        details_desktop:
          "Upewnij się, że nie zasłaniasz dokumentu, w tym jego dolnych krawędzi, palcem. Zwróć też uwagę na refleksy z hologramu na polach dokumentu.",
        title: "Wszystkie pola powinny być widoczne",
        title_desktop: "Wszystkie pola powinny być widoczne",
      },
    },
    next_btn: "Dalej",
  },
  onboarding_modal: {
    aria: "Instrukcje skanowania",
    barcode_only: {
      details:
        "Znajdź kod kreskowy (seria czarnych kresek lub kod kwadratowy). Skieruj na niego aparat i trzymaj nieruchomo — skanowanie nastąpi automatycznie.",
      details_desktop:
        "Znajdź kod kreskowy (seria czarnych linii lub kod kwadratowy). Upewnij się, że obiektyw aparatu jest czysty, a kod kreskowy dobrze oświetlony.",
      title: "Znajdź i zeskanuj kod kreskowy",
      title_desktop: "Wyczyść obiektyw i znajdź kod kreskowy",
    },
    btn: "Rozpocznij skanowanie",
    document_with_barcode: {
      details:
        "Różne typy dokumentów mogą mieć odmienne formaty kodów kreskowych oraz ich umiejscowienie. Obejrzyj przód i tył dokumentu, aby znaleźć kod kreskowy.",
      details_desktop:
        "Sprawdź przód i tył dokumentu, aby znaleźć kod kreskowy. Upewnij się, że obiektyw aparatu jest czysty, a dokument dobrze oświetlony.",
      title: "Zlokalizuj kod kreskowy na dokumencie",
      title_desktop: "Wyczyść obiektyw i znajdź kod kreskowy",
    },
    document_with_mrz: {
      details:
        "Na dole przedniej lub tylnej strony dokumentu znajdziesz długi ciąg znaków, podzielony na 2 lub 3 wiersze i oddzielony strzałkami (<< lub >>).",
      details_desktop:
        "Sprawdź przód i tył dokumentu pod kątem strefy MRZ. Poszukaj 2–3 linii znaków i symboli strzałek (<<) na dole dokumentu. Upewnij się, że obiektyw aparatu jest czysty, a dokument dobrze oświetlony.",
      title: "Znajdź strefę MRZ na dokumencie",
      title_desktop: "Wyczyść obiektyw i zlokalizuj strefę MRZ",
    },
    full_document: {
      details:
        "Upewnij się, że dokument jest dobrze oświetlony. Wszystkie pola dokumentu powinny być widoczne na ekranie aparatu.",
      details_desktop:
        "Upewnij się, że obiektyw aparatu jest czysty, a dokument dobrze oświetlony. Wszystkie pola dokumentu powinny być widoczne na ekranie aparatu.",
      title: "Wszystkie szczegóły powinny być widoczne",
      title_desktop: "Przygotuj się do skanowania",
    },
  },
  sdk_aria: "Ekran skanowania dokumentów",
  timeout_modal: {
    details:
      "Upewnij się, że dokument jest dobrze oświetlony, w pełni widoczny i bez odblasków.",
    details_desktop:
      "Upewnij się, że obiektyw aparatu jest czysty, a dokument w pełni widoczny, wyraźny i dobrze oświetlony.",
    title: "Nie można odczytać dokumentu.",
  },
} as const;
