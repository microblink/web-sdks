...
import type { BlinkCardWorkerInitSettings, ProgressStatusCallback } from "@microblink/blinkcard-worker";
...
describe("loadBlinkCardCore", () => {
  ...
  it("fills default userId and resourcesLocation", async () => {
    ...
    expect(createProxyWorkerMock).toHaveBeenCalledWith(expectedLocation, "blinkcard-worker.js");
    ...
    expect(remoteWorker.initBlinkCard).toHaveBeenCalledWith(settings as BlinkCardWorkerInitSettings, undefined);
  });
  ...
  it("proxies and passes progress callback to init", async () => {
    ...
    expect(remoteWorker.initBlinkCard).toHaveBeenCalledWith(settings as BlinkCardWorkerInitSettings, progressCallback);
  });
});
