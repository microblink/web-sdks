...
/**
 * Parameters for looking up expected WASM/data file size. `buildType` is optional and used only by packages that have
 * full/lightweight variants (e.g. BlinkID).
 */
...
export type SizeManifest = Record<ResourceFileType, Record<WasmVariant, SizeManifestLeaf>>;

export function getWasmFileSize(params: GetWasmFileSizeParams, manifest: SizeManifest): number {
