...
/**
 * Options for image quality settings. Stricter settings will provide better quality images as the scan will not
 * complete until they are met. If not defined internal defaults will be used.
 */
export type ImageQualitySettings = Partial<{
  /** Threshold for detecting blur on the document. Higher levels provide stricter requirements. */
  blurMatchLevel: MatchLevel;
  /** Threshold for detecting glare on the document. Higher levels provide stricter requirements. */
  glareMatchLevel: MatchLevel;

  /** Threshold for expected lighting level of a document on screen. Higher levels provide stricter requirements. */
  lightingMatchLevel: MatchLevel;

  /** Threshold for detecting the sharpness of the document. Higher levels provide stricter requirements. */
  sharpnessMatchLevel: MatchLevel;

  /** Threshold for detecting hand occlusion on the document. Higher levels provide stricter requirements. */
  handOcclusionMatchLevel: MatchLevel;

  /** Threshold for detecting the DPI of the document. Higher levels provide stricter requirements. */
  dpiMatchLevel: MatchLevel;

  /** Threshold for detecting the tilt of the document in screen. Higher levels provide stricter requirements. */
...
/**
 * Represents the level of strictness for a matching check during the document verification process. This enum defines
 * the different levels that can be configured for various checks, such as photocopy or barcode anomaly detection.
 * Higher levels indicate stricter requirements, leading to fewer false positives but potentially more false negatives.
 *
 * - `"disabled"`: The matching check is disabled, that check will not be performed.
 * - `"level-1" to "level-10"`: Increasing levels of strictness, from least to most strict.
 */
 ...
 * for more or less conservative approach.
 *
 * - `"ignore"`: Ensures that BlinkID Verify returns a `Pass` or `Fail` verdict even if image quality is not good enough.
 * - `"conservative"`: If image quality is not good enough, BlinkID Verify will refuse to process the image and a
 *   `NotPerformed` verdict will be returned. When a `NotPerformed` verdict is returned, BlinkID Verify will prompt the
 *   user to repeat the capture process. This is indicated by the `Retry` value under `RecommendedOutcome` in the
 *   response.
 * - `"high-assurance"`: A `Fail` verdict can be returned even if image quality is not good enough, but occasionally a
 *   `Pass` verdict will not be returned if image quality is not satisfactory.
 * - `"high-conversion"`: A `Pass` verdict can be returned even if image quality is not good enough, but occasionally a
 *   `Fail` verdict will not be returned if image quality is not satisfactory.
 * - `"very-high-conversion"`: A `Pass` verdict can be returned even if image quality is significantly unsatisfactory, but
 *   a `Fail` verdict will not be returned if image quality is really poor.
 */
...
/**
 * Defines the strictness of checks performed by BlinkID Verify as part of the `UseCase` settings.
 *
 * - `"permissive"`: Optimized for letting real users through. In cases of doubt or lower confidence, BlinkID Verify will
 *   avoid failing the document.
 ...
 * - `"very-strict"`: Reasonable only for the most sensitive use cases. Significant user friction is added to stop as much
 *   fraud as possible.
 */
export type VerificationPolicy = "permissive" | "standard" | "strict" | "very-strict";
 ...
 * - `"rejected-and-accepted"`: Both rejected and accepted documents will be sent for manual review.
 ...
 * - `"in-person"`: Document verification is performed in an in-person environment in which a trained employee is scanning
 ...
 *
 * - `"no-control"`: _Deprecated_: Is the same as `"basic"`, it will be removed in the future.
