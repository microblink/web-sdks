...
import { loadBlinkIdCore, type BlinkIdScanningResult, type RemoteScanningSession } from "@microblink/blinkid-core";
import {
  createBlinkIdUxManager,
  type BlinkIdUiStateKey,
  type BlinkIdUxManager,
} from "@microblink/blinkid-ux-manager/core";
import { CameraManager } from "@microblink/camera-manager/core";

import "./index.css";

type BlinkIdCore = Awaited<ReturnType<typeof loadBlinkIdCore>>;
...
const UI_STATE_MESSAGES: Partial<Record<BlinkIdUiStateKey, string>> = {
...
function formatUiState(key: BlinkIdUiStateKey): string {
...
function formatResult(result: BlinkIdScanningResult): string {
  const resultWithoutImages = structuredClone(result);

  for (const subResult of resultWithoutImages.subResults) {
...
class BlinkIdCustomUiExample {
  #core?: BlinkIdCore;
  #coreLoad?: Promise<BlinkIdCore>;
  #session?: RemoteScanningSession;
  #cameraManager = new CameraManager();
  #uxManager?: BlinkIdUxManager;
  ...
  async initialize(): Promise<void> {
    ...
    statusElement.textContent = "Loading BlinkID SDK resources…";
    this.#updateControls();
    ...
    try {
      ...
      this.#sdkState = "ready";
      statusElement.textContent = "BlinkID SDK is ready. Start a scanning session.";
    } catch (error) {
      if (!this.#isSdkCurrent(sdkRunId)) {
        return;
      }
      ...
      this.#sdkState = "idle";
      showError(error);
      statusElement.textContent = "Unable to load BlinkID SDK resources.";
    }
  ...
  async start(): Promise<void> {
    ...
    statusElement.textContent = "Creating BlinkID scanning session…";
    this.#updateControls();
  ...
  async destroySdk(): Promise<void> {
    ...
    this.#sessionState = "stopping";
    retryButton.hidden = true;
    statusElement.textContent = "Destroying BlinkID SDK…";
    this.#updateControls();
    ...
    if (this.#isSdkCurrent(sdkRunId)) {
      this.#sdkState = "idle";
      this.#sessionState = "idle";
      statusElement.textContent = "BlinkID SDK destroyed. Load it again to continue.";
      this.#updateControls();
    }
  }
  ...
  #getCore(sdkRunId: number): Promise<BlinkIdCore> {
    if (this.#core) {
      return Promise.resolve(this.#core);
    }

    this.#coreLoad ??= loadBlinkIdCore(
      {
        licenseKey: import.meta.env.VITE_LICENCE_KEY,
      },
      (progress) => {
        if (this.#isSdkCurrent(sdkRunId)) {
          statusElement.textContent = `Loading BlinkID SDK resources… ${progress.progress}%`;
        }
      },
    )
      .then((core) => {
  ...
  async #disposeSession(): Promise<void> {
    ...
    if (session) {
      await session.delete().catch((error: unknown) => {
        console.warn("Failed to delete the BlinkID scanning session.", error);
      });
    }
  }
  ...
  async #terminateCore(): Promise<void> {
    ...
    if (core) {
      await core.terminate().catch((error: unknown) => {
        console.warn("Failed to terminate BlinkID Core.", error);
      });
    }
  }
...
const example = new BlinkIdCustomUiExample();
