...
import { CameraError, CameraManager, CameraManagerComponent, createCameraManagerUi } from "@microblink/camera-manager";
...
/**
 * Configuration for camera UI elements. These settings control which built-in UI components are shown to the user. We
 * disable default error handling to implement custom UI for better user experience.
 */
...
/**
 * Configuration for the document scanning feedback UI. These settings control the visibility of built-in feedback and
 * error UI components. We disable default modals to implement custom UI for better error handling and user guidance.
 */
 ...
 *
 * Features: - Custom error handling for core initialization, camera, and scanning errors - Document type filtering (US
 * passports only in this example) - Custom UI for various scanning states and error conditions - Proper resource
 * cleanup and state management
 */
export const App: Component = () => {
  /**
   * Portal configuration for UI rendering When enabled, UI components will be rendered outside the root element, which
   * can help avoid z-index and styling conflicts with the main app.
   */
   ...
   * Core components
   *
   * These manage the lifecycle of different BlinkID components: - BlinkID Core: Main SDK module with WASM and resources
   * - Scanning Session: Handles document scanning logic. Each session is independent and can be used to scan only one
   * document. - UX Manager: Coordinates UI and scanning operations - Camera Manager: Controls camera access and frame
   * capture
   */
  ...
  const [blinkIdScanningResult, setBlinkIdScanningResult] = createSignal<BlinkIdScanningResult>();

  /**
   * Error state signals for custom UI handling Each signal manages a specific type of error that can occur during: -
   * Core initialization - Camera access - Scanning timeout - Document validation - Document type filtering
   */
  ...
  const [unsupportedDocumentError, setUnsupportedDocumentError] = createSignal<boolean>(false);
  const [filteredDocumentError, setFilteredDocumentError] = createSignal<string | null>(null);

  /** Reset all error states to their initial values. Called before starting new operations to ensure clean state. */
   ...
   *
   * This is the first step in the initialization process: 1. Loads the WASM module and required resources 2. Unlocks
   * the SDK with the license key 3. Updates application state based on initialization result
   *
  ...
  async function initBlinkIdCore() {
    ...
    try {
      ...
      if (!licenseKey) {
        throw new Error("License key is not configured. Please check your environment variables.");
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to initialize BlinkID SDK. Please check your license key.";
   ...
   *
   * This step prepares the SDK for document scanning: 1. Ensures BlinkID Core is initialized 2. Creates a new scanning
   * session for document processing. Each session is independent and can be used to scan only one document.
   *
  ...
  async function initBlinkIdScanningSession() {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "An unexpected error occurred while initializing the session";
   ...
   *
   * This is the main entry point for document scanning that: 1. Ensures a valid scanning session exists 2. Initializes
   * camera and UI components 3. Sets up error handling and event callbacks 4. Configures document filtering (US
   * passports only) 5. Starts the camera stream for scanning
   *
  ...
  async function startBlinkIdScanningSession() {
    ...
    try {
      ...
      blinkIdUxManager = await createBlinkIdUxManager(cameraManager, scanningSession);
      blinkIdUxManager.setTimeoutConfiguration({ inactivityTimeoutMs: 15000 });

      // Create and configure camera UI
      cameraManagerComponent = await createCameraManagerUi(cameraManager, targetNode, CAMERA_UI_CONFIG);

      /**
       * Monitor camera errors and update UI accordingly. Provides real-time feedback about camera access and operation
       * issues.
       *
      ...
      /**
       * Handle scanning process errors. Currently configured to handle timeout scenarios after 15 seconds of scanning.
       *
      ...
      /**
       * Monitor UI state changes to handle unsupported document scenarios. When UNSUPPORTED_DOCUMENT state is detected,
       * show appropriate error UI.
       */
      blinkIdUxManager.addOnUiStateChangedCallback((uiState: BlinkIdUiState) => {
      ...
      /**
       * Configure document type filtering This example demonstrates how to restrict scanning to specific document
       * types. Here we only allow US passports - all other documents will be filtered out and trigger the
       * onDocumentFiltered callback.
       *
       ...
       */
      blinkIdUxManager.addDocumentClassFilter((documentClassInfo: DocumentClassInfo) => {
        return !(documentClassInfo.country?.id === "usa" && documentClassInfo.documentType?.id === "passport");
      });

      /**
       * Handle filtered documents This callback is triggered when a document is detected but filtered out by the
       * document class filter above. It allows us to provide specific feedback to the user about why their document was
       * not accepted.
       *
       * @param documentClassInfo Contains details about the filtered document including its type and country of origin
       */
      blinkIdUxManager.addOnDocumentFilteredCallback((documentClassInfo: DocumentClassInfo) => {
      ...
      /**
       * Process successful scanning results. When a document is successfully scanned: 1. Store the scanning result 2.
       * Stop the camera feed by dismounting the UI
       *
      ...
      /**
       * Monitor camera playback state changes. When playback is ready: 1. Initialize the feedback UI 2. Start frame
       * capture if onboarding guide is not shown 3. Update application state
       *
      ...
      unsubscribePlaybackState = cameraManager.subscribe(
        ...
        (state) => {
          if (state === "playback") {
            createBlinkIdFeedbackUi(blinkIdUxManager!, cameraManagerComponent!, FEEDBACK_UI_CONFIG);
      ...
      /**
       * Clean up resources when camera UI is dismounted. Performs cleanup by: 1. Removing event subscriptions 2.
       * Resetting UI manager and session states 3. Clearing camera component reference
       */
      ...
      const errorMessage =
        error instanceof CameraError ? error.message : "Failed to start camera capturing. Please try again.";
  ...
  return (
    <div class="container">
      <h1>BlinkID UI Overrides Example</h1>

      <p>
        This example demonstrates how to implement custom UI for various error states in the BlinkID SDK. The SDK core
        loads automatically, then click the button to start scanning and see custom error dialogs.
      </p>
      ...
      <div style={{ "margin-bottom": "20px" }}>
        ...
        >
          {loadState() === "loading" ? "Starting..." : "Start BlinkID Scanning Session"}
        </button>
